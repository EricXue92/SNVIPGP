...
def main(hparams):
    ...
    if hparams.sngp:
        ...
        if hparams.conformal_training:
            loss_fn = ConformalTrainingLoss(alpha=hparams.alpha, beta=hparams.beta,
                                            temperature=hparams.temperature, sngp_flag=True)
        else:
    ...
    def compute_test_loss_at_last_epoch(trainer):
        ...
        test_loss = round(metrics["loss"],3)
        ...
        test_accuracy, test_loss, auroc, aupr, inefficiency = round(test_accuracy, 3), round(test_loss, 3), round(auroc, 3), round(aupr, 3), round(inefficiency, 3)
        ...
        result["auroc"] = auroc
        result["aupr"] = aupr
        result['acc'] = test_accuracy
        result['loss'] = test_loss
        result['ineff'] = inefficiency
...
def parse_arguments():
    ...
    assert args.dropout_rate >= 0 and args.dropout_rate < 0.1, "pls remember to avoid fucking underfitting"
    return args 
...
if __name__ == "__main__":
    ...
    # sweep_id = wandb.sweep(sweep=sweep_config, project="IPGP-5-TIMES")
    #
    # ###Step 4: Activate sweep agents
    # wandb.agent(sweep_id, function=partial(run_main, args=args), count=5)
